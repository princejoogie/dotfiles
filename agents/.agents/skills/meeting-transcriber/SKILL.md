...
2. Create a dated output directory, for example `.tmp/meeting-transcriber-YYYY-MM-DD-<slug>/`. Use `scripts/init_meeting_package.py` when a scaffold is helpful.
...
Run helpers from the workspace where the output package should live:

```bash
python /Users/pjuguilon/.agents/skills/meeting-transcriber/scripts/init_meeting_package.py --title "Meeting title" --source /path/to/input.mov
python /Users/pjuguilon/.agents/skills/meeting-transcriber/scripts/probe_media.py /path/to/input.mov --output media-probe.md
python /Users/pjuguilon/.agents/skills/meeting-transcriber/scripts/extract_screenshots.py /path/to/video.mov --out-dir .tmp/meeting/screenshots --timestamp 00:12:34 --timestamp 00:45:10
python /Users/pjuguilon/.agents/skills/meeting-transcriber/scripts/ocr_screenshots.py .tmp/meeting/screenshots --output-dir .tmp/meeting/ocr
python /Users/pjuguilon/.agents/skills/meeting-transcriber/scripts/annotations_to_markdown.py annotations.json --output github-comments.md
```

## Deliverable Package

Prefer this shape when the user asks for a full package:
